...
export default class StackTransitioner extends Component {
  ...
  render() {
    ...
    const childrenStyle = before ? this.getRouteStyle(1) : (
      after ? this.getRouteStyle(0) : styles.stackView
    )
    const previous = (
    ...
    return (
      <View style={styles.stackView} {...this.panResponder.panHandlers}>
        <View style={styles.transitionContainer}>
          <Animated.View key={`upper-${before ? previousLocation.key : location.key}`} style={childrenStyle}>
            {children}
          </Animated.View>
          {after && previous}
        </View>
